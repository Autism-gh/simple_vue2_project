import { isBoolean } from '@/util/common/type-check'
...
export const listToTree = (list, rules = treeNodeOptions['default'], showRules = {}) => {
    ...
    list.forEach(item => {
        ...
        if (!isBoolean(showNode)) {
            ...
            if(disabled !== undefined) ruleOption.disabled = disabled
            
            Object.assign(item, ruleOption)
            map[item[id]] = item
            ruleList.push(item)
        }
    })
    ...
    return {dataObject: treeData, dataArray: ruleList}
}