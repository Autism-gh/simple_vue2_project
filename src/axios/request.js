...
import { getToken } from '@/util/app/auth'
...
/**
 *
 *  老接口没有走这里的 instance 所以未参与计数
 *
 */
let loadingCount = 0
export const changeLoadingState = (options) => {
  if (store) {
    if (options) {
      store.commit('app/TOGGLE_LOADING', true)
    } else {
      loadingCount--
      // 可能有其他请求的接口 未计数导致这里的请求数量偏多
      if (loadingCount <= 0) {
...
const checkNeedLoading = (request) => {
  if (!request.useLoading) {
    loadingCount++
    if (request.useLoading !== undefined) delete request.useLoading
...
export const post = (url, { api = appService, useLoading = false } = {}) => {
  return async data => {
...
export const postForm = (url, { api = appService, useLoading = false } = {}) => {
  return async data => {
    return (await requestCenter[api]({
      method: 'post',
      url,
...
export const postObjToForm = (url, { api = appService, useLoading = false } = {}) => {
  return async data => {
    return (await requestCenter[api]({
      method: 'post',
      url,
...
export const get = (url, { api = appService, useLoading = false } = {}) => {
  return async data => {
    return (await requestCenter[api](
      {
        method: 'get',
